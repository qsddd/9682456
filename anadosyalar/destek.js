const { Client, WebhookClient, RichEmbed } = require('discord.js'),
      { token, support } = require('../ayarlar.json'),
      { guildID, channel, settings } = support,
      { stripIndents } = require('common-tags'),
      yardimciOlduMuMesaj = "\n\nBu size yardımcı oldu muf?",
      client = new Client();
            ...
            if (channelsAndUsersObject.EMBED_IS_NOT_REPLIED === true) {
                if (msg.content === "1" || msg.content === "2" || msg.content === "3" || msg.content === "4" || msg.content === "5" || msg.content === "6") {
                    ...
                    if (msg.content === "1") {
                        ...
                        .setDescription("Botumuzun tüm komutlarına `f?yardım` yazarak ulaşabilirsin." + yardimciOlduMuMesaj)
                        .setColor('RANDOM')
                        .setFooter("© 2018 | Fredux ");
                        ...
                        var embed = new RichEmbed()
                        .setDescription("**" + msg.author.username + "**" + " bu bot, aslında sadece küçük çaplı bir sunucu botu olarak kurulmuştur. Ancak ardından yoğun ilgi görmesi üzerine bir herkese açık bot olmuştur. Sahibi `xDragex#0743`'dir. Kuruluş tarihi 01 Haziran 2018'dir." + yardimciOlduMuMesaj)
                        .setColor('RANDOM')
                        .setFooter("© 2018 | Fredux ")
                        hook.send({ embeds: [embed] }).then(async hookMsg => {
                            client.channels.get(channelsAndUsersObject.CHANNEL_ID).overwritePermissions(channelsAndUsersObject.USER_ID, {
                        ...
                        var embed = new RichEmbed()
                        .setDescription("**" + msg.author.username + "**" + ", bu sunucu Fredux  BOT'un resmi sunucusudur. Fredux  BOT kullanıcılarına destek vermek, tavsiyelerini almak ve sohbet ortamı oluşturmayı amaçlamıştır." + yardimciOlduMuMesaj)
                        .setColor('RANDOM')
                        .setFooter("© 2018 | Fredux")
                        hook.send({ embeds: [embed] }).then(async hookMsg => {
                            client.channels.get(channelsAndUsersObject.CHANNEL_ID).overwritePermissions(channelsAndUsersObject.USER_ID, {
